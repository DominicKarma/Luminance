...
To make your mod use Luminance, add ``modReferences = Luminance`` in your mod's ``build.txt``. In order to be able to reference the mod when programming, download ``Luminance.dll`` from the latest github release, or extract the mod in-game and take the dll from there. Ensure you reference the latest mod version to avoid things breaking.
