...
namespace KarmaLibrary.Core.Graphics.Primitives
{
    public class PrimitiveTrail
    {
        ...
        public void UpdateBaseEffect(out Matrix effectProjection, out Matrix effectView)
        {
            ...
            effectProjection = Matrix.CreateOrthographicOffCenter(0f, (ProjectionAreaWidth ?? Main.instance.GraphicsDevice.Viewport.Width) * zoom.X, 0f, (ProjectionAreaHeight ?? Main.instance.GraphicsDevice.Viewport.Height) * zoom.Y, 0f, 1f) * zoomScaleMatrix;
        ...
        {
            effectProjection = Matrix.CreateOrthographicOffCenter(0, ProjectionAreaWidth ?? Main.instance.GraphicsDevice.Viewport.Width, ProjectionAreaHeight ?? Main.instance.GraphicsDevice.Viewport.Height, 0, 0f, 1f);
        ...
        public void Draw(IEnumerable<Vector2> originalPositions, Vector2 generalOffset, int totalTrailPoints, float? directionOverride = null) => DrawPrimsDirect(originalPositions, generalOffset, totalTrailPoints, false, directionOverride);

        public void DrawPrimsDirect(IEnumerable<Vector2> originalPositions, Vector2 generalOffset, int totalTrailPoints, bool pixelated, float? directionOverride = null)
        {
        ...
        internal void DrawPrimsFromVertexData(VertexPosition2DColor[] vertices, short[] triangleIndices, bool pixelated)
        {
            ...
            Main.instance.GraphicsDevice.ScissorRectangle = new(0, 0, Main.instance.GraphicsDevice.Viewport.Width, Main.instance.GraphicsDevice.Viewport.Height);
